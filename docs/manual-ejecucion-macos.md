...
```zsh
INVOICEOPS_MODE=demo INVOICEOPS_DB_PATH=var/local-demo/invoiceops.db uv run uvicorn invoiceops.legacy.app:app --host 127.0.0.1 --port 8000
```
...
La configuración local de ejemplo usa `var/local-demo/invoiceops.db`. Para usar otra base de datos, defina `INVOICEOPS_DB_PATH` en el mismo comando de inicio. En zsh, las variables se escriben antes del comando y separadas por espacios:
